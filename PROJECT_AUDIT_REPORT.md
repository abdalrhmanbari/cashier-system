...
| AuditLog | سجل تدقيق | ✅ **[تم الحل 2026-07-05]** فُحص شمولاً (Grep كامل) قبل أي تعديل: `lib/audit.ts`/`logAudit()` كانت ميتة بالكامل (صفر استدعاء)، بينما وُجدت 4 نقاط منفصلة تستدعي `prisma.auditLog.create` مباشرة (تجديد/دفعات الاشتراك، صيانة متجر/منصة) لا تغطي أياً من النقاط الحسّاسة المطلوبة. رُبط `logAudit()` الآن بستة نقاط حسّاسة إضافية عبر try/catch صامت (فشل التسجيل لا يُفشل العملية أبداً، ويُسجَّل عبر `lib/logger.ts`): (1) تغيير سعر الصرف `store/exchange-rate` POST، (2) تسديد ديون العملاء `store/customers/[id]` PATCH (مسار `payment`)، (3) تسوية الجرد ADJUSTMENT فقط `store/inventory` POST، (4) إنشاء/تعديل مستخدمي المتجر `store/users` POST/PATCH (بلا كلمات مرور/hashes بالسجل)، (5) `super-admin/admins` POST/PATCH (بلا كلمات مرور، توثيق بالاسم/الإيميل داخل `newData` لأن `AuditLog.userId` مرتبط بـ`StoreUser` فقط وليس `SuperAdmin`)، (6) `PATCH store/settings` (الضريبة/التقريب/pricingCurrency). **اختُبر حياً**: نُفِّذت عملية واحدة من كل نوع عبر API حي (خادم تطوير)، وتحقّقت كل الأكواد المتوقعة (201/200) وسجلات `AuditLog` الناتجة عبر استعلام Prisma مباشر — مطابقة تماماً (لا تسريب كلمات مرور). اختُبر أيضاً الفشل الصامت: عُطِّل `prisma.auditLog.create` مؤقتاً (حقل غير موجود بالـ schema)، فنجحت عملية `exchange-rate` الأصلية (201) رغم فشل التسجيل (مسجَّل عبر `logger.error`)، ثم أُعيد الكود لطبيعته وتحقَّق التسجيل من جديد. لا تعديل على schema. **[تصحيح لاحق 2026-07-05]**: تبيّن عند المراجعة أن الأربع نقاط القديمة (`RECORD_SUBSCRIPTION_PAYMENT`, `RENEW_SUBSCRIPTION`, `ENABLE/DISABLE_STORE_MAINTENANCE`, `ENABLE/DISABLE_MAINTENANCE`) لم تكن مغلَّفة بـ try/catch صامت رغم أنها استُثنيت خطأً من الفحص الأول — أي فشل بها كان سيُفشل الطلب الأصلي بالكامل، عكس المبدأ المطلوب. غُلِّفت الآن الأربعة بنفس نمط try/catch + `logger.error` (بلا أي تعديل آخر على منطقها)، واختُبرت حياً: `POST /api/super-admin/stores/[id]/subscription/renew` أعاد **200** وأنشأ سجل `AuditLog` جديد بنجاح (`RENEW_SUBSCRIPTION`, id `cmr7pdlnn001coor6bcyfb1xo`). **✅ [تم 2026-07-05] `AuditLog.superAdminId`**: أُضيف حقل اختياري (`String?`) بعلاقة اختيارية مع `SuperAdmin` (تعديل schema بجلسة مخصصة، مع `npm run db:backup` قبل `db:push`). عُدِّلت النقاط الست الخاصة بالسوبر أدمن (`admins` POST/PATCH + الأربع القديمة المغلَّفة أعلاه) لتملأ `superAdminId` بدل الاعتماد فقط على الاسم/الإيميل داخل `newData` — الاسم/الإيميل أُبقيا بـ`newData` كتوثيق إضافي، لا ترحيل للسجلات القديمة (تبقى `superAdminId: null` فيها، مقبول). **اختُبر حياً**: `POST subscription/renew` أعاد 200 وأنشأ سجل `AuditLog` (id `cmr7q4juq0021uiamzwwxriwn`) بـ`superAdminId: "cmr62lzgz000096j8o1sgwog6"` مطابق فعلياً لحساب `admin@platform.com` المستخدَم بالاختبار. |
...
- ✅ **[تم الحل 2026-07-05] سقف الخصم التجاري القابل للضبط لكل دور** — أُضيف حقلان اختياريان بـ `Store` (`maxDiscountPercentCashier`, `maxDiscountPercentManager`، نقاط أساس بنفس اصطلاح `taxRate`، `null` = بلا سقف). `app/api/store/sales/route.ts` يحسب الآن نسبة الخصم الإجمالية الفعلية (خصومات الأصناف اليدوية + خصم الفاتورة معاً) نسبةً إلى إجمالي الأصناف قبل أي خصم يدوي (`subtotalSyp + totalItemDiscounts`)، ويقارنها بسقف دور المستخدم الحالي (`t.role`) — تجاوز يُرفض بـ 400 ورسالة عربية تذكر النسبتين (المطلوبة والمسموحة). طبقة منع السالب من 2026-07-04 لم تُمس. الواجهة: تبويب "سقف الخصم" جديد بصفحة `settings` (مدير فقط)، وتنبيه UX بـ `pos/page.tsx` يعطّل زر تأكيد البيع محلياً عند تجاوز الكاشير سقفه (الفرض الحقيقي بالسيرفر فقط). **اختُبر حياً**: بيع عادي بلا خصم وبخصم صغير (201 بنفس الأرقام السابقة تماماً — صفر انحراف)، سقف كاشير 10%/مدير 30% ← كاشير 15% (400 برسالة "15.0% تتجاوز السقف المسموح لدورك 10.0%") ← نفس الـ15% بجلسة مدير (201) ← إزالة السقف (null) ← نفس الـ15% بجلسة كاشير (201 مجدداً).
...
### 4.6 ✅ [تم الحل 2026-07-05] انجراف تقريب بسيط في المرتجعات الجزئية المتكررة

كانت المشكلة: `app/api/store/returns/route.ts` يحسب نصيب كل استرجاع جزئي بـ `Math.round((saleItem.netLineTotal * reqItem.quantity) / saleItem.quantity)` — يُعاد من الصفر (بالنسبة لكامل `saleItem.quantity` الأصلية) بكل عملية استرجاع منفصلة، فقد يتراكم فرق تقريب صغير عبر دفعات متعددة لنفس السطر.

**الحل المنفَّذ**: عند استرجاع دفعة تُكمل كامل الكمية المتبقية للسطر (`alreadyReturned + reqItem.quantity === saleItem.quantity`)، تُحسب `netRefund`/`taxRefund` كـ"المتبقي المالي بالضبط" (`saleItem.netLineTotal`/`lineTax` ناقص مجموع ما استُرجع فعلياً بالدفعات السابقة لنفس `SaleItem`، مُشتقّاً من `SaleReturnItem.total - taxRefund` و`taxRefund` بالتوالي) بدل الحساب النسبي. الدفعات غير الأخيرة بقيت بالحساب النسبي الحالي بلا أي تغيير. لا تعديل على schema (استُخدمت حقول `SaleReturnItem` الموجودة فقط).

**اختُبر حياً**: فاتورة (INV-2026-0011، 3 قطع، خصم كسري 100 ل.س) بـ `netLineTotal=108650`, `lineTax=0` — غير قابلة لقسمة صحيحة على 3. استُرجعت على 3 دفعات منفصلة (قطعة واحدة بكل مرة): `36217` + `36217` + `36216` = **108650** بالضبط، مطابق تماماً لـ`netLineTotal + lineTax` بلا أي انجراف (فرق سنت واحد كان سيتراكم بالحساب النسبي القديم لو استمر النمط).
...
### 6.1 ✅ [تم الحل 2026-07-05] فجوة: انتهاء الاشتراك لا يمنع استدعاء الـ API مباشرة

كانت المشكلة: تحقق `Subscription.status`/`endDate` موجوداً فقط بـ `app/[slug]/(store)/layout.tsx` (خاص بعرض الصفحات)، وليس في `requireStore()` (`lib/store-auth-helper.ts`) ولا في middleware، فكان متجر اشتراكه منتهٍ (`EXPIRED`) أو معلَّق (`SUSPENDED`/`CANCELLED`) **يبقى قادراً فعلياً على تنفيذ عمليات API كاملة** (بيع، مرتجع، إغلاق وردية...) طالما الجلسة صالحة والطلب يذهب مباشرة لـ `/api/store/*` بدل المرور بصفحة محجوبة.

**الحل المنفَّذ**: أُضيف تحقق `Store.isActive` + `Subscription.status`/`endDate` داخل `requireStore()` نفسه (`lib/store-auth-helper.ts`)، بإعادة استخدام `syncExpiredStatus()` الموجودة أصلاً بـ `lib/subscription.ts` (بلا تكرار منطق). عند الحجب يُرجَع `403 JSON` برسالة عربية تميّز بين السببين:
- `"تم إيقاف هذا المتجر من قبل الإدارة"` — عند `Store.isActive === false`.
- `"انتهت صلاحية اشتراك المتجر، يرجى التجديد للمتابعة"` — عند اشتراك `EXPIRED`/`SUSPENDED`/`CANCELLED` أو `ACTIVE` تجاوز `endDate`، أو عدم وجود اشتراك أصلاً.

طُبِّق كاش قصير الأمد (45 ثانية، بمفتاح `storeId`، بنفس روح كاش الصيانة `STATUS_CACHE_MS` بـ `middleware.ts`) لتفادي استعلام Prisma إضافي بكل طلب API. بما أن `requireManager()` مبني فوق `requireStore()`، فهو يرث هذا التحقق تلقائياً بلا أي تعديل عليه. طبقة `layout.tsx` (فحص حي بلا كاش لكل تحميل صفحة) بقيت كما هي دون مساس — تُستخدم الآن كطبقة أولى للواجهة، بينما `requireStore()` هي طبقة الفرض الفعلية على مستوى كل الـ API.

**نتائج الاختبار الحي (خادم تطوير، 2026-07-05)**:
- متجر فعّال: `sales`/`shifts`/`returns`/`reports`/`customers`/`products`/`settings` كلها 200 كالسابق لكل من الكاشير والمدير.
- `Store.isActive = false` (يدوياً): كل `/api/store/*` (باستثناء `ping`) ترجع `403` مع رسالة "تم إيقاف هذا المتجر من قبل الإدارة" — تحققت لكل من الكاشير والمدير.
- `Subscription.status = EXPIRED` (يدوياً): نفس المسارات ترجع `403` مع رسالة "انتهت صلاحية اشتراك المتجر، يرجى التجديد للمتابعة".
- إعادة الحالة إلى طبيعتها (`isActive = true`, `status = ACTIVE`): الطلب مباشرة بعد الاستعادة يبقى `403` (الكاش لم ينتهِ بعد)، ثم بعد ~45 ثانية يعود `200` تلقائياً بلا إعادة تشغيل — الكاش ينتهي فعلياً كما هو متوقَّع.
- `ping` والمصادقة (`store-auth`) وصفحات الحالة العامة لم تتأثر (لا تمر بـ `requireStore`).
...
2. **✅ [تم الحل 2026-07-05]** لم يكن يوجد فرض لحالة الاشتراك (منتهي/معلَّق) على مستوى الـ API — متجر مقطوع الاشتراك كان يبقى قادراً على البيع والتحصيل عبر استدعاء مباشر للـ API بينما الواجهة تحجبه فقط بصرياً. أُضيف الآن التحقق داخل `requireStore()` نفسه مع كاش 45 ثانية ورسالة 403 مميِّزة، واختُبر حياً. (قسم 6.1)
...
2. **✅ [تم 2026-07-05]** نقل تحقق `Subscription.status`/`endDate`/`Store.isActive` إلى داخل `requireStore()` نفسه، بدل الاكتفاء بحجبه على مستوى الصفحة فقط. انظر تفصيل الحل والاختبار بالقسم 6.1.
...
6. **✅ [تم جزئياً 2026-07-05، أُكمل جزء منه 2026-07-05]** تدقيق (audit) تعديلات حسابات السوبر أدمن المتبادلة مُفعَّل فعلياً (`CREATE_SUPER_ADMIN`/`UPDATE_SUPER_ADMIN` بـ `super-admin/admins` POST/PATCH)، و**`AuditLog.superAdminId`** أُضيف فعلياً (راجع تفصيل AuditLog بالقسم 2) — عُدِّلت النقاط الست الخاصة بالسوبر أدمن لملء هذا الحقل بدل الاعتماد على `newData` JSON وحده. **يبقى معلَّقاً:** مستوى صلاحية "جذر" فعلي يمنع تعديل حساب لآخر (التدقيق يسجّل التعديل الآن لكن لا يمنعه بعد) — خارج نطاق جلسات schema المنجزة.
10. **ملاحظة تنظيف بعد اختبار AuditLog (2026-07-05)**: اختبار `POST /api/store/exchange-rate` الحي أنشأ سجلين تجريبيين بجدول `ExchangeRate` (append-only، لم يُحذفا: `15000` ثم `15100`) أصبح آخرهما السعر الحي الفعلي للمتجر. صُحِّح ذلك بتسجيل سعر صحيح جديد فوقه (`14500`) عبر نفس الـ API — السجلان التجريبيان باقيان بالتاريخ كسجل عادي (النظام append-only بالتصميم، لا حذف). سجلات `AuditLog` الناتجة عن كل الاختبارات (exchange-rate/customers/inventory/users/admins/settings/subscription renewal) تُركت كما هي عمداً كطلب صريح. **[تحقّق لاحق 2026-07-05]**: جلسة سقف الخصم/superAdminId/تقريب المرتجعات لم تُنشئ أي سجل `ExchangeRate` جديد — السعر الحي بقي `14500` كما تُرك.
7. **✅ [تم 2026-07-05]** تحديث `PROJECT_OVERVIEW.md` ليعكس كل الفروقات المذكورة بالقسم 8 — تم بجلسة توثيق منفصلة (بلا أي تعديل كود)، راجع القسم 10 أدناه.
...
## 10. تحديث التوثيق — 2026-07-05

جلسة توثيق فقط (بلا أي تعديل كود أو schema أو أوامر db) أعادت كتابة `PROJECT_OVERVIEW.md` بالكامل ليطابق كل فروقات القسم 8 أعلاه، تحديداً:

- تصحيح كل ما وُصف "مخطط/فارغ/غير منفذ" فعلياً: `InventoryMovement` (API + صفحة)، `stores/[id]/subscription` (صفحة + 3 routes)، `returns/page.tsx`، نظام الإشعارات (محدَّث إلى 10 أنواع حسب القسم 6.2)، وضع الصيانة بمستوييه، نظام الاشتراكات، وحدة المراقبة والتسجيل.
- تصحيح آلية حماية الكاشير إلى `CASHIER_ALLOWED` (سماح) + `CASHIER_API_RULES` بمنطقها الصحيح.
- توثيق إصلاحات 2026-07-04: تحقق الخصم سيرفر-سايد، نظام backup/restore.
- تصحيح حالة الأوفلاين: `lib/db/offline.ts`/`sync.ts` موصوفان الآن بوضوح ككود ميت غير موصول (وليس "قيد التنفيذ").
- إضافة قسم "المتبقي" الصريح: فرض الاشتراك داخل `requireStore` (أولوية قصوى)، فحص `AuditLog`، سقف الخصم القابل للضبط، انجراف تقريب المرتجعات الجزئية.
- إضافة الكود الميت المكتشف حديثاً لقائمة Legacy: `lib/db/offline.ts`, `lib/db/sync.ts`, `formatCurrency()`, تبعيتَي `zustand` و`@zxing/library`.

جميع الفحوصات السريعة لوجود الملفات المذكورة بالتقرير (قبل الاعتماد عليها بالتوثيق الجديد) أُعيدت بتاريخ 2026-07-05 وتأكَّدت مطابقتها للواقع الحالي — لا تغيّر شيء منذ 2026-07-04.

**البند 7 من قائمة المهام المقترحة (القسم 9) أصبح ✅ منجزاً.**

**2026-07-05: دُمجت الازدواجية — CLAUDE.md صار قواعد فقط، PROJECT_OVERVIEW.md المرجع المعماري الوحيد.** (ملاحظة: لم يكن هناك CLAUDE.md فعلي بالمستودع وقت هذا الدمج — أُنشئ من الصفر بصيغته القصيرة النهائية مباشرة، بعد التأكد أن قسم العملة الثنائية موسَّع بالكامل داخل `PROJECT_OVERVIEW.md`.)
